...
for(i in 1:100) { 
    base_plot_a <- base_plot_a + rA(100) %>% 
      stat_smooth(mapping = aes(x=x, y=y), method='lm', se=FALSE, formula='y~x', color = 'grey', size = 0.5, alpha = 0.5)
}
...
for(i in 1:100) { 
    base_plot_b <- base_plot_b + rB(100) %>% 
      stat_smooth(mapping = aes(x=x, y=y), method='lm', se=FALSE, formula='y~x', color = 'grey', size = 0.5, alpha = 0.5)
  }
...
for(i in 1:100) { 
    base_plot_c <- base_plot_c + rC(100) %>% 
      stat_smooth(mapping = aes(x=x, y=y), method='lm', se=FALSE, formula='y~x', color = 'grey', size = 0.5, alpha = 0.5)
}
...
## any frequentist test has what components: numerator and denominator? 

numerator   <- 0.14
denominator <- diag(sqrt(vcovHC(model_one)))['sqrft']

test_stat <- numerator / denominator

test_stat

pt(test_stat, df = 87, lower.tail = FALSE) * 2
```

```{r}
## we could do it by hand... or... let someone else do it for us
