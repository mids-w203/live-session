...
```{r}
tone_of_voice     <- 'irritating'
mode_of_speech    <- 'yell'
superlative       <- 'most'
score_on_test     <- '90 percent' # should end with percent
name_of_classmate <- 'bob'
emotion           <- 'angry'
eating_verb       <- 'chug' #slurp
vessel            <- 'kettle'
thing_found_in_compost <- 'melon rind'
```

## Madlib completed

Suppose that a classmate comes to you, and, in a `r tone_of_voice` voice `r mode_of_speech`, "Hey, I've got something that is `r superlative` for statistics test prep. All you've got to do to get `r score_on_test` on Test 2 and make `r name_of_classmate` `r emotion` is `r eating_verb` this `r vessel` of `r thing_found_in_compost`.
...
```{r}
home_team <- read_csv('./data/home_team.csv') %>% 
...
t.test(home_team$out_state_tuition_diff)
