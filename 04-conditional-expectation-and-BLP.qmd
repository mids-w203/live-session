...
y <- (x-3)^3 + 3*x - .25*(x^2 * (x-3)) + rnorm(n=10000, mean=0, sd=3)
...
> Today's most advanced machine learning algorithms *cannot possibly* perform better than the conditional expectation function at making a prediction.
>
> **📣 At no time ever** will a machine learning algorithm perform better than the conditional expectation. 📣
...
2.  **Produce** the conditional expectation function as a predictor, given joint densities of random variables.
3.  **Appreciate** that the best linear predictor, which is a restriction of predictors to include only those that are linear combinations of variables, can produce reasonable predictions and **anticipate** that the BLP forms the target of inquiry for regression.

## Class Announcements

### Test 1 is releasing to you this week.
...
### Compare Expectation and Conditional Expectation

1.  **Think back by a week**: What desirable properties of a predictor does the expectation possess? What makes these properties desirable?
2.  **Think about this week**: How, if at all, does the conditional expectation improve on these desirable properties? How does it accomplish this?

### Contrast Expectation and Conditional Expectation

-   Contrast $E[Y]$ and $E[Y|X]$. For example, when you look at how these operators are "shaped", how are their components similar or different?[^04-conditional-expectation-and-blp-1]

-   What is $E[Y|X]$ a function of? What are "input" variables to this function?

-   What, if anything, is $E[E[Y|X]]$ a function of?

[^04-conditional-expectation-and-blp-1]: Note, when we say "shaped" here, we're referring to the deeper concept of a statistical functional. A statistical functional is a function of a function that maps to a real number. So, if $T$ is the functional that we're thinking of, $\mathcal{F}$ is a family of functions that it might operate on, and $\mathbb{R}$ is the set of real numbers, a statistical functional is just $T: \mathcal{F} \rightarrow \mathbb{R}$. The Expectation statistical functional, $E[X]$ always has the form $\int x f_{X}(x)dx$.)
...
ggplot() + 
  ...
  geom_segment(aes(x = 0, y = 0, xend = 0.5, yend = 0.66)) + 
  labs(
...
-   What is the conditional variance function?[^04-conditional-expectation-and-blp-2]

[^04-conditional-expectation-and-blp-2]: Take a moment to strategize just a little bit before you get going on this one. There is a way to compute this value that is easier than another way to compute this value.
...
### Working with a gnarly function and a lot of data. 
...
```{r im going to fit a moving average}
