...
::: {.callout-note name="Why So Serious?"}
...
The short answer, is that the we use this $\leq r$ range because we're using a [Boreal $\sigma$ algebra](https://en.wikipedia.org/wiki/Borel_set) to constrain the sets that have probability measures. Why this constraint? If we don't weird stuff can happen. Like, real weird things: you can split a sphere into pieces and create two new spheres of equal volume from the pieces ([Banch-Tarski Paradox](https://en.wikipedia.org/wiki/Banach–Tarski_paradox)).
:::
